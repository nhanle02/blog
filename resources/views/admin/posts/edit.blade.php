    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Chỉnh sữa</h3>
            </div>
            @if (session('error'))
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" value="{{ old('title', $post->title) }}" class="form-control" id="title" placeholder="Nhập tiêu đề">
                                    @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="slug" value="{{ old('slug', $post->slug) }}" class="form-control" id="slug" placeholder="Nhập slug">
                                    @error('slug')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" id="content" name="content" class="form-control" cols="30" rows="5">{{ old('content', $post->content) }}</textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="show_img">
                                        @if (!empty($post->image))
                                            <img src="{{ asset($post->image) }}" alt="ảnh bài viết" width="200px" height="200px">
                                        @endif
                                    </div>
                                </div>
                            </div>
                                    ...
                                    @foreach ($tags as $tag)
                                        <div class="custom-control custom-checkbox">
                                            ...
                                            @foreach ($post->tags as $post_tag)
                                                {{ $tag->id == $post_tag->id ? 'checked' : '' }}
                                            @endforeach
                                            value="{{ $tag->id }}" name="tags[]" type="checkbox" id="tag--{{ $tag->id }}">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" {{ old('status', $post->status) == '1' ? 'checked' : '' }} name="status" class="custom-control-input" id="status">
                                        <label class="custom-control-label" for="status">Trạng thái</label>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" {{ old('comment_status', $post->comment_status) == '1' ? 'checked' : '' }} name="comment_status" class="custom-control-input" id="comment_status">
                                        <label class="custom-control-label" for="comment_status">Trạng thái comment</label>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" {{ old('is_featured',$post->is_featured) == '0' ? 'checked' : '' }} name="is_featured" class="custom-control-input" id="is_featured">
